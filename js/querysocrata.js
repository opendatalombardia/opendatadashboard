...
function mappingfor () {
        for (i = 0; i < datasets.length; i++) {
            ...
            function getData( url, query, selector, tilename ) {
               $.get(url + "?" + query, function(data) {
                  $(selector)
                   .append('<div class="col-lg-3 col-md-3 col-sm-6 col-xs-12">' +
                              '<div class="tile">' +
                                '<div class="tile-content">' +
                                 '<div class="tile-label">' + tilename + '</div>' +
                                 '<div class="tile-description"><p class="' + tilename + '">' + data[0].count + '</p></div>' +
                                 '<div class="tile-footer"><a href="' + url + '" target="_blank">Vai al dataset »</a></div>' +
                              '</div></div></div>'
                           );
            });
            };
